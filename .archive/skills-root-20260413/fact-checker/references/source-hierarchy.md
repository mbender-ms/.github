...
> REST API specs on GitHub are treated as **ground truth** for API parameters, defaults, and schemas ΓÇö equivalent to Tier 1 for API-specific claims.
...
### Tier 5 ΓÇö Internal Documentation
...
### Tier 6 ΓÇö Internal Code & Configuration
...
### Tier 7 ΓÇö Internal Product Metadata
...
1. **Always prefer the highest available tier** ΓÇö Don't cite Tier 4 if Tier 1 covers the same claim.
2. **Internal sources never appear in public docs** ΓÇö Internal findings go in a separate confidential section.
3. **Contradictions resolve upward** ΓÇö Higher tier wins unless the lower tier is more recent AND from an authoritative Microsoft source.
4. **Unverifiable is acceptable** ΓÇö Flag claims that can't be verified rather than removing them.
5. **Date matters** ΓÇö A newer Tier 2 blog post may supersede an older Tier 1 doc page. Note the dates.
6. **REST API specs are ground truth for APIs** ΓÇö GitHub REST specs (Tier 2) are authoritative for API parameters and schemas.
