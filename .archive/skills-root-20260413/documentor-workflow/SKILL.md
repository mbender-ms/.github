...
description: "Editorial quality workflows for Azure documentation ΓÇö SEO optimization, metadata generation, engagement analysis, markdown auto-fix, sensitive identifier replacement, and link validation. Replicates DocuMentor extension capabilities for use in agent mode or without the extension."
...
# Documentor Workflow ΓÇö Editorial Quality for Azure Docs
...
4. Scan for passive voice ΓÇö rewrite to active voice using "you" to address readers
5. Check for sensitivity issues ΓÇö replace any real GUIDs, secrets, or IDs with approved placeholders (see `references/sensitive-identifiers.md`)
6. Verify sentence-style capitalization on all headings (except product names)
7. Check that procedures have Γëñ 10 steps and begin with a verb
...
4. Verify writing style against Microsoft Writing Style Guide:
   ...
   - Get to the point fast ΓÇö lead with what matters most
   - Use bigger ideas, fewer words
   - Write like you speak ΓÇö read it aloud
...
1. **Title** ΓÇö 30-65 chars, title case, primary keyword, unique from H1/description
2. **Description** ΓÇö 120-165 chars, CTA, primary keyword at beginning, unique
3. **H1** ΓÇö Sentence case (NOT title case), primary keyword, no gerunds
4. **Intro paragraph** ΓÇö Primary keyword in first or second sentence
5. **Subheadings** ΓÇö Sentence case (NOT title case), secondary keywords, no gerunds, preserve standard template headings (Prerequisites, Related content, Next steps)
6. **Image alt text** ΓÇö 40-150 chars, starts with "Screenshot of..." or "Diagram of..."
...
1. **Bounce rate** ΓÇö Check intro hooks, visual elements, content relevance, page load factors
2. **Click-through rate** ΓÇö Check link placement, anchor text quality, CTA clarity
3. **Copy-try-scroll rate** ΓÇö Check code sample quality, procedure clarity, scanability
4. **Dwell rate** ΓÇö Check content depth, readability, multimedia, internal linking
5. **Exit rate** ΓÇö Check next steps section, related content links, progressive disclosure
...
2. For relative links ΓÇö verify the target file exists in the repository
3. For absolute links ΓÇö flag any links to `learn.microsoft.com` that should be relative
4. For anchor links ΓÇö verify the heading target exists
...
- `references/seo-checklist.md` ΓÇö Complete SEO optimization rules and examples
- `references/metadata-rules.md` ΓÇö Title, description, and customer-intent specifications
- `references/engagement-checklist.md` ΓÇö Engagement metric troubleshooting and remediation
- `references/sensitive-identifiers.md` ΓÇö Approved GUID/secret replacement values by type and severity
- `references/autofix-rules.md` ΓÇö Markdown formatting rules and code fence language identifiers
