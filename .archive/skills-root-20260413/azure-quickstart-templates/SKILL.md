...
# Azure Quickstart Templates ΓÇö Contribution & Authoring Skill
...
- Credentials (usernames, passwords, secrets) ΓÇö **never** use defaultValues for these
...
- Do **not** overuse `allowedValues` for inclusive lists (e.g., all VM SKUs) ΓÇö only for exclusive choices
...
- Avoid concatenating variable names for conditional scenarios ΓÇö use template expressions and dictionary objects
...
- **Never** hard-code resource endpoints or properties ΓÇö use `reference()` (JSON) or symbolic property access (Bicep)
...
- Must use Azure Marketplace or core platform images ΓÇö no custom images
...
1. **Badge images** ΓÇö Public/Gov test dates and results, Best Practice, Cred Scan, Bicep Version
2. **Deploy to Azure** button
3. **Deploy to Azure US Gov** button
4. **Visualize** button
5. **Description** of what the template deploys
6. **Tags** ΓÇö comma-separated in backticks: `` `Tag1, Tag2, Tag3` ``
...
- When using Bicep, do **not** include `azuredeploy.json` in the PR ΓÇö it is auto-built on merge
- No absolute URLs in samples ΓÇö use `_artifactsLocation` / `uri()` pattern for deployment artifacts
...
- **Never** output secrets (passwords, account keys) ΓÇö outputs may be visible to read-only users

---

## 13. Checklist ΓÇö Reviewing a Template
...
- [ ] `metadata.json` exists with valid schema, display name (Γëñ60 chars), summary (Γëñ200), description (Γëñ1000)
...
- [ ] Bicep element order is correct (targetScope ΓåÆ params ΓåÆ vars ΓåÆ resources ΓåÆ outputs)
...
## 14. Checklist ΓÇö Creating a New Template
