---
mode: agent
description: Research a Microsoft topic with citations and source tiers ΓÇö no file editing
tools:
...
1. **Topic** ΓÇö What do you want to research?
2. **Product area** ΓÇö Which Microsoft product area? (Azure, M365, Security, Power Platform, Dynamics 365, Windows, DevTools)
3. **Specific questions** ΓÇö Any particular claims or facts you want verified?
4. **Output format** ΓÇö Chat only, file only, or both?

## Step 1 ΓÇö Broad search

Search across official sources:
1. `microsoft_docs_search` ΓÇö Multiple queries targeting different angles of the topic
2. Use product-area-specific search paths from SKILL.md ΓåÆ Product Area Search Domains
3. Note all potentially relevant pages with URLs and tiers

## Step 2 ΓÇö Deep retrieval

For the most relevant sources:
1. `microsoft_docs_fetch` ΓÇö Retrieve full page content
2. `microsoft_code_sample_search` ΓÇö Find official code samples if applicable
3. Extract key facts with their exact source locations
4. Note publication dates and last-updated dates

## Step 3 ΓÇö Cross-reference and verify
...
- **Verified** ΓÇö Confirmed by Tier 1 or Tier 2 source
- **Partially verified** ΓÇö Confirmed with caveats or incomplete coverage
- **Unverifiable** ΓÇö No authoritative source found
- **Conflicting** ΓÇö Sources disagree (note which sources and tiers)

## Step 5 ΓÇö Deliver output
