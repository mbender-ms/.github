...
Use answers to select search domains and load the matching sources catalog YAML from `sources/` (e.g., `azure-networking.yml` for Azure Networking). This gives you the full list of relevant GitHub repos for Tier 2 source verification.
...
Always prefer the highest available tier. See [source-hierarchy.md](references/source-hierarchy.md) for the complete reference with repository catalog integration.
