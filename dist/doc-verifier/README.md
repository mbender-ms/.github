...
> **Set up the Microsoft Learn MCP Server** (required): add the following to your VS Code MCP config (`.vscode/mcp.json` in a workspace, or your user `mcp.json`), then restart VS Code:
>
> ```json
> {
>   "servers": {
...
> ```
>
> Without this server the `microsoft_docs_search` and `microsoft_docs_fetch` tools are unavailable and every workflow fails. See the [Microsoft Learn MCP docs](https://learn.microsoft.com/training/support/mcp) for the latest setup guidance.
...
This is a **self-contained** bundle — every file the skill needs is inside this folder. There are no external skill dependencies to copy separately.
...
├── references/
│   ├── source-hierarchy.md                      # Tiered source authority reference (vendored)
│   ├── quality-checklist.md                     # Fact-check quality gate (vendored)
│   ├── source-guide.md                          # Educational guide to sources
│   └── workflows.md                             # Detailed per-workflow procedures
└── sources/
    ├── routing-index.md                         # Product area → category lookup
    ├── categories-index.yml                     # Category catalog
    └── *.yml                                     # Per-org / per-category repo catalogs (Tier 2)
```

> **Self-contained note**: `references/source-hierarchy.md`, `references/quality-checklist.md`, and the `sources/` catalog are vendored copies of files that live in a shared location in the source repository. They are bundled here so the skill works standalone.
