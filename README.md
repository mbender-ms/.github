...
│   ├── beta-feedback-synthesizer.agent.md  # Triages beta feedback into revision plan
│   ├── editing-pipeline.agent.md        # Orchestrates full post-draft editing pipeline
...
| Agent | Description |
|-------|-------------|
| `@ai-decontaminator` | Scans fiction manuscripts or non-fiction articles for AI-generated text signatures — vocabulary fingerprints, punctuation tics, structural tells, prose clichés, emotional scaffolding, false depth markers, and dialogue flattening. v2.0: 10 detection categories, 14 rewrite rules. Produces a severity-rated flagged report (🔴/🟡/🟠) with an overall AI-signal score, then remediates every flagged passage to restore authentic human voice. Works on a single file or an entire manuscript in parallel |
...
── Post-Draft Editing (editing-pipeline orchestrator) ────────────────
1. originality-manuscript-checker → originality-fixer (if needed)
2. fiction-editor (5-pass: character → continuity → universe → prose → polish)
3. beta-romantasy & beta-scifi (parallel)
4. beta-feedback-synthesizer (triages beta reports → revision plan)
5. fiction-editor (revision mode — targeted fixes from synthesis)
6. ai-decontaminator (scan + remediate AI patterns)
...
| `@beta-feedback-synthesizer` | Triages parallel beta reader reports into a single prioritized revision plan. Classifies feedback by severity (critical/major/minor/polish), cross-references overlapping issues, and produces actionable revision instructions for fiction-editor's revision mode |
| `@editing-pipeline` | Sonnet-based orchestrator that sequences the full post-first-draft editing pipeline: originality check → 5-pass fiction edit → parallel beta reads → feedback synthesis → targeted revision → AI decontamination. Extracts sub-agent reports and manages handoffs automatically |
