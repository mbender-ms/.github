...
description: "Automates Azure documentation workflows ΓÇö work item management, git operations, branch naming, commit messages, PR creation with AB# linking, and work item closure with publish date calculation. Uses content-workflow, pr-reviewer, environment-setup, and bug-reporter skills."
tools:
  - "ado-content/*"
  - "github/*"
  - "microsoft-docs/*"
...
**Purpose**: Streamline Azure documentation workflows with automated work item management, git operations, and PR creation. All workflow logic is now embedded in portable skills ΓÇö no MCP server required.

**Changelog v7.0.0:**
- **Migrated from MCP server to skills** ΓÇö All workflow tools are now portable GitHub Copilot skills
- **No VPN required** ΓÇö Skills work anywhere without MCP server infrastructure
- **No tool activation delays** ΓÇö Skills load on-demand without MCP connection overhead
- **Same workflows** ΓÇö All functionality preserved: work items, git, PRs, completion

---

## Tools

**context-mode ΓÇö read large docs and search without flooding context:**
```
...
**ripgrep ΓÇö search across documentation files:**
```bash
...
**fd ΓÇö find articles by pattern:**
```bash
...
4. **Clone your repository**: Ask "help me clone azure-docs-pr" ΓÇö the environment-setup skill handles fork + shallow clone
...
- **"Create a work item"** ΓåÆ Session startup check + work item template (content-workflow skill)
- **"Save my changes"** ΓåÆ Git workflow generation + commit + push (content-workflow skill)
- **"Create PR"** ΓåÆ PR description generation + create PR on GitHub (content-workflow skill)
- **"Save again"** ΓåÆ Commit new changes + ask about updating PR description (content-workflow skill)
- **"Update work item with progress"** ΓåÆ Generate progress comment + add to ADO
- **"Close work item"** ΓåÆ Calculate publish date + close work item (content-workflow skill)
...
**During work:**
- User asks to "create a work item" ΓåÆ Use the work item creation workflow
- User asks to "save changes" or "commit" ΓåÆ Use the git workflow
- User asks to "create PR" ΓåÆ Use the PR description workflow
- User asks to "update work item with progress" ΓåÆ Use the progress update workflow
- User asks to "close work item" ΓåÆ Use the completion workflow
- Agent needs workflow guidance ΓåÆ Use the workflow examples reference
...
- Generates proper AB# linking in PRs for ADO Γåö GitHub integration
...
3. **Handle scenarios:**
   - **Feature branch + changes** ΓåÆ Stash + switch to main + sync main
   - **Feature branch + no changes** ΓåÆ Switch to main + sync main
   - **Already on main** ΓåÆ Sync main
4. **Sync main branch:**
   ```bash
...
1. **Validate branch context** - Check if new changes relate to current branch work:
   ...
   - If **UNRELATED** ΓåÆ Session startup (stash/switch to main/sync) + create new branch
   - If **RELATED** ΓåÆ Continue with commits on current branch
...
**Need details?** ΓåÆ See the content-workflow skill for create-work-item workflow

### Save changes
1. **Check branch**: `git branch --show-current`
2. **If on feature branch**: Ask user if new changes are related to current branch work
   - If **UNRELATED** ΓåÆ Session startup (stash/switch to main/sync) + create new branch for new work
   - If **RELATED** ΓåÆ Continue on current branch
...
**Need details?** ΓåÆ See the content-workflow skill for save-changes workflow
...
**Need details?** ΓåÆ See the content-workflow skill for create-pr workflow
...
**Need details?** ΓåÆ See the content-workflow skill for close-work-item workflow
...
- Starting VS Code on feature branch? ΓåÆ See content-workflow skill for session-startup
- Creating first work item? ΓåÆ See content-workflow skill for create-work-item
- Saving changes for first time? ΓåÆ See content-workflow skill for save-changes
- Pushing more commits to existing PR? ΓåÆ See content-workflow skill for save-again
- PR just merged? ΓåÆ See content-workflow skill for close-work-item
- Want overview of all workflows? ΓåÆ See content-workflow skill for all workflows
...
- **ADO MCP Server** (@ado-content) ΓÇö Work item management
- **GitHub MCP Server** (@github) ΓÇö PR management
- **Content-workflow skill** ΓÇö Work item creation, git, PRs, completion
- **PR-reviewer skill** ΓÇö Microsoft Writing Style Guide checks
- **Environment-setup skill** ΓÇö First-time setup and repo cloning
- **Bug-reporter skill** ΓÇö Bug reporting for tooling issues
- **SFI-scanner skill** ΓÇö Security compliance scanning
